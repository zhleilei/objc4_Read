...
// RefcountMap disguises(伪装) its pointers because we
...
storeWeak(id *location, objc_object *newObj)
{
    ...
    // Order by lock address to prevent(防止) lock ordering problems. 
 ...
 * 此函数将新值存储到__weak变量中。它会可在__weak变量是赋值目标的任何地方使用。
...
/*
  Once upon a time(很久以前) we eagerly(急切地) cleared *location if we saw the object
  was deallocating. This confuses(混淆) code like NSPointerFunctions which
  tries to pre-flight the raw(原始) storage and assumes(假设) if the storage is
  zero then the weak system is done interfering(干扰). That is false: the
  weak system is still going to check and clear the storage later. 
  This can cause objc_weak_error complaints(投诉) and crashes.
  So we now don't touch the storage until deallocation completes. 因此，在取消分配完成之前，我们现在不要触摸存储。
*/
...
#   define POOL_BOUNDARY nil  /// 边界
    ...
    static void * operator new(size_t size) {
        return malloc_zone_memalign(malloc_default_zone(), SIZE, SIZE); //内存对齐
    }
    /// 重载delete运算符
...
#warning zll tls? 英文为Thread Local Storage [1]  ，缩写为TLS。为什么要有TLS？原因在于，全局变量与函数内定义的静态变量，是各个线程都可以访问的共享变量。
